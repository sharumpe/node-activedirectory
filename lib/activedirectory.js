...
ActiveDirectory.prototype.getUsersForGroup = function getUsersForGroup(opts, groupName, callback) {
  ...
  self.findGroup(_.defaults({}, _.omit(opts || {}, 'attributes'), {
                 ...
                 groupName, function(err, group, foundIn) {
    if (err) {
      if (callback) callback(err);
      return;
    }
      ...
      search.call(self, localOpts, function onSearch(err, members) {
        ...
        async.forEach(members, function(member, asyncCallback) {
          ...
          if (! member.groupType) {
            ...
            // TDM: Add this item to the foundIn array
            foundIn = foundIn || {};
            foundIn[user.dn] = foundIn[user.dn] || {};
            // Add the groupName to the foundIn, but avoid duplicates
            foundIn[user.dn][groupName] = true;
            log.info('Noting that user %s was found in group %s', user.dn, groupName);
            ...
            if ( foundIn && foundIn[member.dn] && foundIn[member.dn][groupName]) {
              // Already seen this one.  Membership Loop?
              log.debug('Skipping previously traversed member::group: %s :: %s', member.dn, groupName);
              asyncCallback();
            }
            else if ( member.dn == groupName ) {
              // They are the same.  Membership Loop?
              log.debug('Skipping equivalent member::group: %s :: %s', member.dn, groupName);
              asyncCallback();
            }
            else {
              // ...add it to the list
              var result = new Group(pickAttributes(member, (opts || {}).attributes || defaultAttributes.group));
              self.emit(result);
              groups.push(result);

              // TDM: Add this item to the foundIn array
              foundIn = foundIn || {};
              foundIn[result.dn] = foundIn[result.dn] || {};
              // Add the groupName to the foundIn, but avoid duplicates
              foundIn[result.dn][groupName] = true;
              log.debug('Noting that user %s was found in group %s', result.dn, groupName);

              // ...recursively get the users belonging to this group.
              self.getUsersForGroup(opts, member.dn, function(err, nestedUsers, nestedGroups, nestedFoundIn) {

                users.push.apply(users, nestedUsers);
                groups.push.apply(groups, nestedGroups);

                // TDM Combine the nestedFoundIn into foundIn
                if ( nestedFoundIn ) {
                  for (var nestattr in nestedFoundIn ) {
                    for (var grattr in nestedFoundIn[nestattr]) {
                      foundIn = foundIn || {};
                      foundIn[nestattr] = foundIn[nestattr] || {};
                      foundIn[nestattr][grattr] = true;
                    }
                  }
                }

                process.nextTick(function() {
                  // Just allow node to do its thing.
                });
                asyncCallback();
              });
            }
...
*/
      ...
      log.info('%s foundIn: %s', groupName, JSON.stringify(foundIn,null,'  '));
      if (callback) callback(null, users, groups, foundIn);
    });
  });
};
