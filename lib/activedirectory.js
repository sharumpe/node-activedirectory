      ...
      search.call(self, localOpts, function onSearch(err, members) {
        ...
        async.forEach(members, function(member, asyncCallback) {
          ...
          if (! member.groupType) {
            ...
            if ( foundIn[user.dn].indexOf(groupName) == -1 ) {
              foundIn[user.dn].push(groupName);
            }
            log.debug('Noting that user %s was found in group %s', user.dn, groupName);
            ...
            self.getUsersForGroup(opts, member.dn, function(err, nestedUsers, nestedFoundIn) {
              ...
              if ( nestedFoundIn ) {
                var keys = Object.keys(nestedFoundIn);
                for(var i=0; i<keys.length; i++) {
                  var key = keys[i];
                  foundIn = foundIn || {};
                  foundIn[key] = foundIn[key] || [];
                  foundIn[key].push.apply(foundIn[key],nestedFoundIn[key]);
                }
              }
